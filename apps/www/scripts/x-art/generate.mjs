...
const TOKENS = {
  ...
  "#fff": "var(--ds-background-100)",
  "#FFFFFF": "var(--ds-background-100)",
  "#ffffff": "var(--ds-background-100)",
  "#F2F2F2": "var(--ds-gray-100)",
  "#f2f2f2": "var(--ds-gray-100)",
}
const CLASS = {
  "text-fg-primary": "",
  "text-fg-secondary": "text-gray-900",
  "text-fg-tertiary": "text-gray-600",
  "text-fg-primary/50": "text-gray-1000/50",
}
...
const toJsx = async (html) => {
  html = html
    .replace(
      /class="fill-neutral-0 dark:fill-neutral-1100"/g,
      'fill="var(--ds-gray-100)"'
    )
    .replace(
      /class="fill-\[#09090b\] dark:fill-\[#f6f6f4\]"/g,
      'fill="currentColor"'
    )
    ...
    ([TOKENS, CLASS]) => {
      ...
      document.querySelectorAll("*").forEach((e) => {
        for (const a of e.attributes) {
          const m =
            a.value.match(/url\(#([^)]+)\)/) ||
            (a.name.endsWith("href") && a.value.match(/^#(.+)$/))
      ...
      const identity = (t) =>
        /^(none|scale\(1\)|scaleX\(1\)|scaleY\(1\)|rotate\(0(deg)?\)|translate\(0(px)?(, ?0(px)?)?\)|translateX\(0(px)?\)|translateY\(0(px)?\)|scale\(1, ?1\)|scale\(1\) translate\(0px, 0px\)|translate\(0px, 0px\) scale\(1\))$/.test(
          t.trim()
        )
      const styleObj = (s) => {
        const o = {}
        ...
        if (o.transform && (identity(o.transform) || isLoop(o.transform)))
          delete o.transform
      ...
      const attrs = (el) => {
        ...
        for (const a of el.attributes) {
          ...
          if (n === "style") {
            ...
            out.push(
              `style={{ ${keys.map((k) => `${camel(k)}: ${esc(token(o[k]))}`).join(", ")} }}`
            )
            continue
          }
          if (n === "class") {
            v = v
              ...
              .map((c) => CLASS[c] ?? c)
              .filter(Boolean)
              .filter((c) => /^text-gray-/.test(c))
              .join(" ")
            if (!v) continue
          }
          if (n === "stroke-dashoffset" && !el.hasAttribute("pathLength"))
            continue // loop phase
          if (n === "opacity" && v === "1") continue
          if (n.endsWith("href") && /^#/.test(v)) {
            usesId = true
            out.push(`${camel(n)}={\`#\${id}-${slug(v.slice(1))}\`}`)
            continue
          }
          v = token(v)
          if (/url\(#/.test(v)) {
            out.push(`${camel(n)}={\`${idRef(v)}\`}`)
            continue
          }
          if (n === "pathLength") {
            out.push(`pathLength={${v}}`)
            continue
          }
      ...
      const walk = (node) => {
        ...
        if (tag === "image") {
          const a = ["x", "y", "width", "height"]
            .filter((k) => node.hasAttribute(k))
            .map((k) => `${k}=${esc(node.getAttribute(k))}`)
            .join(" ")
        ...
        if (tag === "foreignObject") {
          ...
          const spans = [...node.querySelectorAll("span")].filter(
            (s) =>
              !/opacity:0/.test(s.getAttribute("style") || "") &&
          ...
          for (const s of parts) {
            const w = (s.getAttribute("style") || "").match(
              /font-weight:(\d+)/
            )?.[1]
            const c = /x-fg-secondary/.test(s.getAttribute("style") || "")
              ? "var(--ds-gray-900)"
              : null
            ...
            segs.forEach((t, i) => {
              ...
              if (t.trim()) cur.push({ t, w, c })
            })
          }
          lines.push(cur)
          const n = lines.length
          const y0 = y + (h - lh * n) / 2 + lh * 0.72
          const body = lines
            .map(
              (l, i) =>
      ...
      const rootAttrs = [...svg.attributes]
        .filter((a) => keep.includes(a.name))
        .map((a) => `${camel(a.name)}=${esc(a.value)}`)
        .join(" ")
      const inner = [...svg.childNodes].map(walk).join("")
      return {
        jsx: `<svg ${rootAttrs} aria-hidden="true" {...props}>${inner}</svg>`,
        usesId,
      }
    },
    [TOKENS, CLASS]
...
for (const m of manifest) {
  ...
  for (const v of m.variants) {
    const html = v.key.endsWith(".svg")
      ? cardart[v.key]
      : catalogue[v.key].outer
  ...
  const vtype = single
    ? ""
    : fns.map((f) => JSON.stringify(f.variant)).join(" | ")
  const body = single
