...
```bash
npx shadcn@latest add @ziiz/pagination
```
...
<RegistrySource name="pagination" />
...
Renders a [`Button`](/components/button) as an `<a>`: `outline` when active, `ghost` otherwise. `PaginationPrevious` and `PaginationNext` accept the same props plus `text`.
