...
```bash
npx shadcn@latest add @ziiz/message-scroller
```
...
```bash
npm install @hugeicons/core-free-icons @hugeicons/react @shadcn/react
```

<Step>Copy and paste the following code into your project.</Step>

<RegistrySource name="message-scroller" />
...
<ComponentPreview
  ...
  previewClassName="p-4 min-[480px]:p-8 min-[560px]:p-10 sm:px-10 sm:py-16"
/>
...
They are identical for the styled component and the unstyled parts, except
that `MessageScrollerButton` also accepts Button `variant`, `size` and `shape`
props (defaults `"secondary"`, `"icon-sm"` and `"default"`).
