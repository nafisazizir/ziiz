...
for (const f of fs.readdirSync("live")) {
  ...
  for (const s of arr) {
    ...
    if (
      !it.outer ||
      (visible && (!it.outerVisible || +width > it.outerWidth))
    ) {
      it.outer = s.outer
      it.outerVisible = visible
      it.outerWidth = +width
    }
  }
}
const list = [...items.values()]
  .map((it) => ({ ...it, pages: [...it.pages], parents: [...it.parents] }))
...
console.log(
  ...
  list.length,
  "settled-visible:",
  list.filter((x) => x.outerVisible).length
)
for (const it of list)
  console.log(
    ...
    it.seen
      .filter((s) => s.rw)
      .map((s) => `${s.width}:${s.rw}x${s.rh}`)
      .slice(0, 3)
      .join(" "),
    "| foreignObject:",
    (it.outer.match(/<foreignObject/g) || []).length,
    "image:",
    (it.outer.match(/<image/g) || []).length
  )
    ...
    (m, a) =>
      `<svg${a
        ...
        .replace(
          /height="[^"]*"/,
          ""
        )} width="${Math.min(1100, it.w * Math.min(1100 / it.w, 600 / it.h))}">`
  )
...
for (let i = 0; i < list.length; i += 12) {
  ...
  await p2.screenshot({
    path: `montage/${String(i / 12).padStart(2, "0")}.png`,
    fullPage: true,
  })
}
await b2.close()
