...
```bash
npx shadcn@latest add @ziiz/alert-dialog
```
...
<RegistrySource name="alert-dialog" />
...
Both render a [`Button`](/components/button). `AlertDialogAction` accepts all of its props; `AlertDialogCancel` accepts its `variant`, `size` and `shape`.

| Prop      | Type                     | Default                                    |
| --------- | ------------------------ | ------------------------------------------ |
| `variant` | `Button["variant"]`      | `"default"` (Action), `"outline"` (Cancel) |
